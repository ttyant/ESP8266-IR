...
extern uint8 buffer[128];

#ifdef DBG
#define pr(...) do { \
		os_sprintf(buffer, __VA_ARGS__); \
...
int ICACHE_FLASH_ATTR MATCH(unsigned int measured, unsigned int desired) {
	pr("!! %d %d[%d - %d]", measured, desired, TICKS_LOW(desired), TICKS_HIGH(desired));
	if((measured >= TICKS_LOW(desired)) && (measured <= TICKS_HIGH(desired))) {
		pr("ok\r\n");
		return 1;
	}else{
		pr("??\r\n");
...
#define MATCH_MARK(a, b) MATCH(a, b)
#define MATCH_SPACE(a, b) MATCH(a, b)
...
long ICACHE_FLASH_ATTR
	decodeSony(decode_results *results) {
  long data = 0;
...
long ICACHE_FLASH_ATTR
	decodeSanyo(decode_results *results) {
  long data = 0;
...
// Looks like Sony except for timings, 48 chars of data and time/space different
long ICACHE_FLASH_ATTR
	decodeMitsubishi(decode_results *results) {
...
int ICACHE_FLASH_ATTR
	getRClevel(decode_results *results, int *offset, int *used, int t1) {
...
long ICACHE_FLASH_ATTR
	decodeRC5(decode_results *results) {
...
long ICACHE_FLASH_ATTR
	decodeRC6(decode_results *results) {
...
long ICACHE_FLASH_ATTR
	decodePanasonic(decode_results *results) {
...
long ICACHE_FLASH_ATTR
	decodeLG(decode_results *results) {
    long data = 0;
    int offset = 1; // Skip first space
...
long ICACHE_FLASH_ATTR
	decodeJVC(decode_results *results) {
    long data = 0;
    int offset = 1; // Skip first space
    // Check for repeat
...
long ICACHE_FLASH_ATTR
	decodeSAMSUNG(decode_results *results) {
  long data = 0;
  int offset = 1; // Skip first space
  // Initial mark
...
int ICACHE_FLASH_ATTR
	compare(unsigned int oldval, unsigned int newval) {
  if (newval < oldval * .8) {
    return 0;
  } 
 ...
 */
long ICACHE_FLASH_ATTR
	decodeHash(decode_results *results) {
...
int ICACHE_FLASH_ATTR
	ir_decode(unsigned int *pbuf, int len, decode_results *results) {
  results->rawbuf = pbuf;
  results->rawlen = len;
#ifdef DBG
{
