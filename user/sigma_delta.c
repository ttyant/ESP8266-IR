...
#define GPIO_SIGMA_DELTA_NUM  16  //defined in gpio register.xls
...
sigma_delta_setup(uint32 GPIO_MUX,uint32 GPIO_NUM,uint32 GPIO_FUNC)
{
    ...
    GPIO_REG_WRITE(GPIO_PIN_ADDR(GPIO_ID_PIN(GPIO_SIGMA_DELTA_NUM)), 
...
void ICACHE_FLASH_ATTR
	set_sigma_div_duty(uint8 div, uint8 duty)
{
    //freq = 80000 (khz) /256 /duty_target * (prescale+1)

    set_sigma_target(duty);//SET DUTY TARGET
    set_sigma_prescale(div);//SET CLK DIV
}

int ICACHE_FLASH_ATTR
	setPwm(int on, int div, int duty)
{
	if(on) {
		sigma_delta_setup(PERIPHS_IO_MUX_GPIO2_U, 2, FUNC_GPIO2);
		set_sigma_div_duty(div, duty);
	} else {
		sigma_delta_close(2);
	}
