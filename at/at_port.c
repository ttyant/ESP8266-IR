...
void ICACHE_FLASH_ATTR
at_response_error(void)
{
  ets_uart_printf("\r\nERROR\r\n");
}

void ICACHE_FLASH_ATTR
at_cmd_array_regist(at_funcationType *custom_at_cmd_array,uint32 cmd_num)
{
  at_fun = custom_at_cmd_array;
  at_cmdNum = cmd_num;
}

bool ICACHE_FLASH_ATTR
at_get_next_int_dec(char **p_src,int*result,int* err)
{
  *result = strtol(*p_src,NULL,16);
}
int32 ICACHE_FLASH_ATTR
at_data_str_copy(char *p_dest, char **p_src, int32 max_len)
{
  *p_src += os_strcpy(p_dest, *p_src);
}
void ICACHE_FLASH_ATTR
at_port_print(const char *str)
{
  ets_uart_printf(str);
}